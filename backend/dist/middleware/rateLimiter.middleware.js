...
exports.ipRateLimiter = exports.rateLimiters = exports.createRateLimiter = void 0;
...
const createRateLimiter = (options) => {
    const { windowMs, max, message = 'Too many requests, please try again later.', statusCode = 429, skipSuccessfulRequests = false, skipFailedRequests = false, keyGenerator = defaultKeyGenerator, handler, blockDuration = 0 } = options;
    return (req, res, next) => {
        ...
        if (entry.blocked && entry.blockExpiry && entry.blockExpiry > now) {
            ...
            logger_1.logger.warn(`Rate limit block active for ${key}`);
            if (handler) {
        ...
        if (entry.requests.length >= max) {
            ...
            if (blockDuration > 0) {
                ...
                logger_1.logger.warn(`Rate limit exceeded for ${key}. Blocking for ${blockDuration}ms`);
            }
            else {
        ...
        if (skipSuccessfulRequests || skipFailedRequests) {
            ...
            res.json = function (data) {
                const shouldSkip = (skipSuccessfulRequests && res.statusCode >= 200 && res.statusCode < 300) ||
