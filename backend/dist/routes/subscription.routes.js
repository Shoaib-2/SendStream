...
const createTrialSession = async (req, res, _next) => {
    try {
...
const getSubscriptionStatus = async (req, res, _next) => {
...
const cancelSubscription = async (req, res, _next) => {
    // Check if user exists in request
    if (!req.user || !req.user.id) {
        res.status(401).json({
            status: 'error',
...
const updateRenewal = async (req, res, _next) => {
    // Check if user exists in request
    if (!req.user || !req.user.id) {
        res.status(401).json({
            status: 'error',
    ...
    try {
        ...
        try {
            ...
            const updatedSubscription = await stripe.subscriptions.update(subscriptionId, { cancel_at_period_end: cancelAtPeriodEnd });
            res.status(200).json({
                status: 'success',
                data: {
                    subscription: {
